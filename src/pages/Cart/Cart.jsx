...
export default function Cart() {
  ...
  const { getLoggeedCart, deleteSpecificItem, updateProduct, setNumOfCartItems } = useContext(CartContext)
  async function getData() {
    let { data } = await getLoggeedCart();
    setAllDate(data.data);
  ...
  return (
    <div className='container'>
      <h2 className='text-2xl text-blue-500 text-center font-bold mt-6'>Shopping Cart</h2>
      <h4 className='text-center mb-6 mt-4 text-xl'>Total Price <span className='font-bold'>{allData.totalCartPrice}  EGP</span></h4>
      <div className='flex justify-end'>
        <Link to={"/checkout"} className='bg-blue-400 p-2 rounded block ms-auto my-2'>Checkout</Link>
